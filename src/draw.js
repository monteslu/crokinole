...
], function(rotateVector, board, boardImg, distance, scalePoints, translatePoints, radiansFromCenter){
  ...
  return function(context){
    ...
    if(im.mouseAction.position && this.selectedDisc){
      ...
      if(this.selectedDisc.onSurface){
        ...
        var length = Math.min(distance(dPx, im.mouseAction.position), this.MAX_IMPULSE_PIXELS);
    
        var impPerc = length / this.MAX_IMPULSE_PIXELS;
        var angle = radiansFromCenter(dPx, im.mouseAction.position) + Math.PI;
        ...
        var holeDist = distance(board.unscaledGoalPt, im.mouseAction.position);
        ...
        context.strokeStyle = style;
        context.beginPath();
        context.arc(im.mouseAction.position.x, im.mouseAction.position.y, 20, 0, Math.PI * 2, true);
        context.closePath();
        context.stroke();
      }
